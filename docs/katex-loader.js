 ...
 * Caches which pages need KaTeX so repeat visits load instantly. Auto-runs on import;
 * call configure({ katexVersion }) before the next microtask to set the version.
 */
class KatexLoader {
    ...
    /** Dispatches the katex-ready event on document. */
    dispatchKatexReady() {
        document.dispatchEvent(new Event('katex-ready'));
    }

    /**
     * Single line for KaTeX: split on newlines, join with space, collapse runs of space to one, ltrim/rtrim.
     * @param {string} raw
     * @returns {string}
     */
    formulaForRender(raw) {
        return raw
            .split(/\r?\n/)
            .join(' ')
            .replace(/\s+/g, ' ')
    ...
    /**
     * Runs the loader: registers katex-ready and DOMContentLoaded listeners, loads KaTeX when needed.
     */
    run() {
...
const loader = new KatexLoader();

/**
 * Set options before auto-run. Call synchronously after import to apply before run.
 * @param {{ katexVersion?: string, maxCachedPages?: number }} options
 */
...
/** Defer run so configure() can be called in the same script. */
queueMicrotask(() => {
    loader.run();
});
